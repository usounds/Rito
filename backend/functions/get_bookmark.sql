-- FUNCTION: public.get_bookmark(text)

-- DROP FUNCTION IF EXISTS public.get_bookmark(text);
...
BEGIN
  ...
  WHERE b.did = p_did
  ORDER BY  b.indexed_at
  GROUP BY b.uri, b.subject, b.ogp_title, b.ogp_description, b.ogp_image, b.created_at, b.indexed_at, b.verified;
END;
$BODY$;

ALTER FUNCTION public.get_bookmark(text)
    OWNER TO rito;

GRANT EXECUTE ON FUNCTION public.get_bookmark(text) TO PUBLIC;

GRANT EXECUTE ON FUNCTION public.get_bookmark(text) TO rito;

GRANT EXECUTE ON FUNCTION public.get_bookmark(text) TO web_anon;
