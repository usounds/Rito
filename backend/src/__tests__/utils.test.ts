...
describe('isRitoPostCandidate', () => {
    const candidate = {
        $type: 'app.bsky.feed.post',
        facets: [{
            features: [{ $type: 'app.bsky.richtext.facet#tag', tag: 'rito.blue' }],
        }],
        embed: {
            $type: 'app.bsky.embed.external',
            external: { uri: 'https://example.com/article' },
    ...
    it('accepts a rito.blue post with an external link', () => {
        expect(isRitoPostCandidate(candidate)).toBe(true);
    });

    it('rejects an unrelated post before queueing', () => {
        expect(isRitoPostCandidate({ ...candidate, facets: [] })).toBe(false);
    });

    it('rejects posts generated by Rito', () => {
        expect(isRitoPostCandidate({ ...candidate, via: 'Rito' })).toBe(false);
    });

    it('rejects posts without an external link', () => {
        expect(isRitoPostCandidate({ ...candidate, embed: undefined })).toBe(false);
    });
